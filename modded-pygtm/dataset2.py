...
class trajectory:
    ...
    def create_segments(self, T):
        ...
        """
        self.T = T
        oversampling = 12  # times per days
...
#         print("I the 1st is", len(I)) > getting 9999
        I = np.insert(I, [0, len(I)], [-1, len(self.ids) - 1])
#         print("I the 2nd is", len(I)) > getting 10001
...
#                 print("t_i" ,t_i) ### last value for t_i is 737122.04166667 so we are losing the last 9 data points after it
                # add segments points to global list
                length = len(x_i) - offset ### 397, why? maybe because -9 for the hr difference (0.04166 +9/12 = 0.79166) 
                                           ### and -12 for the first day difference ie offset
#                 print("length:", length, "\n", "x_i", x_i.size)
                if x_i.size: # this is the loop that jumps in T (prt), I think the loop should be outside this one
                    x0[ptr : ptr + length] = x_i[0:-offset] # without the last 12 items, keeping the size for both 0 and t in check
                    y0[ptr : ptr + length] = y_i[0:-offset]
                    xt[ptr : ptr + length] = x_i[offset:] # without the first 12 items
                    yt[ptr : ptr + length] = y_i[offset:]
                    ptr += length
#                     print(ptr) ### increases in 397 which is the size of prt
